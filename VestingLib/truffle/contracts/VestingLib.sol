...
library VestingLib {
  ...
  function initializeETHBalance(VestingStorage storage self, uint256 _balance) returns (bool) {
  ...
  function initializeTokenBalance(VestingStorage storage self, CrowdsaleToken token, uint256 _balance) returns (bool) {
    require(msg.sender == self.owner);
    require(now < self.startTime);
    require(_balance != 0);
    require(self.isToken);
                        ...
                        uint256 _bonus) returns (bool)
  {
    ...
    if(err){
      LogErrorMsg(_totalAmount,"Not enough left in contract to allocate this amount!");
      return false;
    }
  ...
  function getPercentReleased(VestingStorage storage self) constant returns(uint256) {
    require(now > self.startTime);
